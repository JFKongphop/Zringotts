...
/**
 * @title ZringottsRealProofTest
 * @notice Integration tests with REAL ZK proofs generated via FFI
 * @dev This tests the complete flow with actual cryptographic proofs
 */
contract ZringottsRealProofTest is Test {
  ZringottsOracle public lending;
  ...
  function setUp() public {
    ...
    lending = new ZringottsOracle(
      IVerifier(address(verifier)), MERKLE_TREE_HEIGHT, weth, usdc, IConnectOracle(address(0)), "", ""
    );
    ...
    vm.label(address(lending), "ZringottsOracle");
  ...
  function test_RealProof_InitialDeposit() public {
    ...
    // publicSignals[0] is new_note_hash from the circuit
  ...
  function test_RealProof_FullFlow() public {
    ...
    lending.deposit(
      ...
      bytes32(0), // new_will_liq_price (not oracle-checked yet)
      ts,
